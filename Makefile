...
DOCKER_IMAGE_NAME ?= ghcr.io/0xjingle/traverse
...
build-%:
	RUSTFLAGS="-C link-arg=-lgcc -Clink-arg=-static-libgcc" \
		cross build --bin traverse --target $* --features "$(FEATURES)" --profile "$(PROFILE)"
...
define docker_build_push
	...
	cp $(BUILD_PATH)/x86_64-unknown-linux-gnu/$(PROFILE)/traverse $(BIN_DIR)/amd64/traverse

	$(MAKE) build-aarch64-unknown-linux-gnu
	mkdir -p $(BIN_DIR)/arm64
	cp $(BUILD_PATH)/aarch64-unknown-linux-gnu/$(PROFILE)/traverse $(BIN_DIR)/arm64/traverse
