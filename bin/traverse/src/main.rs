//! # Traverse
//!
//! Traverse is a testnet OP Stack rollup aimed at enabling experimentation of bleeding edge
...
use clap::Parser;
use eyre::Context;
use traverse_node::{chainspec::TraverseChainSpecParser, node::TraverseNode};
    ...
    if let Err(err) =
        Cli::<TraverseChainSpecParser, RollupArgs>::parse().run(|builder, rollup_args| async move {
            let node = builder
                ...
                .with_components(TraverseNode::components(rollup_args.clone()))
