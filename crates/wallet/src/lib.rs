...
impl<Provider, Eth> TraverseWallet<Provider, Eth> {
...
#[async_trait]
impl<Provider, Eth> TraverseWalletApiServer for TraverseWallet<Provider, Eth>
where
    ...
    async fn send_transaction(&self, mut request: TransactionRequest) -> RpcResult<TxHash> {
        ...
        if let Some(authorizations) = &request.authorization_list {
            ...
            if authorizations.iter().any(|auth| !valid_delegations.contains(&auth.address)) {
                return Err(TraverseWalletError::InvalidAuthorization.into());
            }
        }
        ...
        match (request.authorization_list.is_some(), request.to) {
            ...
            (false, Some(TxKind::Call(addr))) => {
                let state =
                    self.inner.provider.latest().map_err(|_| TraverseWalletError::InternalError)?;
            ...
            _ => return Err(TraverseWalletError::IllegalDestination.into()),
        }
        ...
        if estimate >= U256::from(350_000) {
            return Err(TraverseWalletError::GasEstimateTooHigh { estimate: estimate.to() }.into());
        }
        request.gas = Some(estimate.to());

        let (base_fee, _) = base_fee.map_err(|_| TraverseWalletError::InvalidTransactionRequest)?;
        ...
        let envelope =
            ...
            .await
            .map_err(|_| TraverseWalletError::InvalidTransactionRequest)?;
